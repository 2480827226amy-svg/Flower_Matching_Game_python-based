...
def level1():
    """
    This function helps to draw the page of level one, also, people can play level one and see whether they win the game or not.
    """
    ...
    pot_1 = Image(Point(opening.WIDTH*2.5/9, opening.HEIGHT*0.6),"flower_pot.gif")
    pot_1.draw(opening.win)

    pot_2 = Image(Point(opening.WIDTH*4.5/9, opening.HEIGHT*0.6),"flower_pot.gif")
    pot_2.draw(opening.win)

    pot_3 = Image(Point(opening.WIDTH*6.5/9, opening.HEIGHT*0.6),"flower_pot.gif")
    pot_3.draw(opening.win)

    position=[(277.1 , 280),(359.1 , 205),(444.1 , 280),(567 , 280),(648 , 205),(733 , 280),(855.88, 280),(936.88, 205),(1021.88, 280)]
    flowerPosition=[]
    flowerColor=[]
    flowerObject = []
    for i in range (3):
        ...
        flower = Image(Point(a[0],a[1]),"blue.gif")
    ...
    for i in range (3):
        ...
        flower = Image(Point(a[0],a[1]),"pink.gif")
        flower.draw(opening.win)
        flowerPosition.append(a)
    ...
    for i in range (3):
        ...
        flower = Image(Point(a[0],a[1]),"purple.gif")
        flower.draw(opening.win)
        flowerPosition.append(a)
        position.remove(a)
    ...
    if checkSuccess1(flowerDic):
        print("You Win!")
        opening.win.getMouse()  
        opening.win.close()    

    for i in range (3):
        number=Text(Point(100, opening.HEIGHT-550),str(3-i))
        number.setSize(30)
        ...
        x=getFlowerIndex(click1,flowerPosition)

        click2=opening.win.getMouse()
        y=getFlowerIndex(click2,flowerPosition)
...
def getFlowerIndex(click, flowerPosition): 
    """
    ...
    for position in flowerPosition:
        ...
        if abs(click.getX()-x)<= 48 and abs(click.getY()-y) <=48:
            return i
        i+=1
    return None

def checkSuccess1(flowerDic):
    """
    ...
    second = [c[:-1] for c in sorted_colors[3:6]]
    third = [c[:-1] for c in sorted_colors[6:9]]

    if all(x == first[0] for x in first) and all(x == second[0] for x in second) and all(x == third[0] for x in third): #https://docs.python.org/3/library/functions.html#all The all() function here can help us to check if all flowers in the current group share the same color at the same time.
        box = Rectangle(Point(opening.WIDTH/2 - 200, opening.HEIGHT/2 - 80),Point(opening.WIDTH/2 + 200, opening.HEIGHT/2 + 80))
